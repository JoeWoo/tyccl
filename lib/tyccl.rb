require File.expand_path("../tyccl/version", __FILE__)
require "algorithms"
require "yaml"
require "singleton"



Pairs = Struct.new(:value,:X_id,:Y_id)


class Tyccl

  include Singleton

  def initialize()
    #read the cilin.txt to codes[] and items[]
    codes=[]
    items=[]
    @codesIndex = Hash.new
    f = File.new(File.expand_path("../cilin.txt", __FILE__))
    i=0
    f.each { |line|
      line.force_encoding('utf-8')
      m=line.split(" ")
      codes << m[0]
      @codesIndex[m[0]] = i
      i += 1
      word = Array.new
      m[1..-1].each{ |term|
        word << term
      }
      items << word
    }
    #init Trie of cilin.txt
    @codesTrie = Containers::Trie.new
    i=0
    codes.each{ |key|
      @codesTrie[key]=items[i]
      i+=1
    }
    #init index of cilin.txt
    @index = YAML::load(File.open(File.expand_path("../Inverted.yaml", __FILE__)))
  end

  def get_codes_sum
  	@codesIndex.size
  end

  def get_index_sum
  	@index.size
  end

  def get_atom_by_id(code)
    @codesTrie[code]
  end

  def get_ids_by_regex(regex)
    @codesTrie.wildcard(regex)
  end

  def get_ids_by_word(word)
    @index[word]
  end

  def has_same?(word)
    codes = get_ids_by_word(word)
    i=0
    flag=false
    while i < codes.size && flag==false  do
      if codes[i][-1]=="="
        flag=true
      else
        flag=false
      end
      i+=1
    end
    return flag
  end

  def has_equal?(word)
    codes = get_ids_by_word(word)
    i=0
    flag=false
    while i < codes.size && flag==false  do
      if codes[i][-1]=="#"
        flag=true
      else
        flag=false
      end
      i+=1
    end
    return flag
  end

  def has_single?(word)
  	codes = get_ids_by_word(word)
    i=0
    flag=false
    while i < codes.size && flag==false  do
      if codes[i][-1]=="@"
        flag=true
      else
        flag=false
      end
      i+=1
    end
    return flag
  end

  def get_same(word)
    if has_same?(word)
      same_words=[]
      codes = get_ids_by_word(word)
      codes.each{ |code|
        if code[-1]=="="
         same_words << get_atom_by_id(code)
        end
      }
      return same_words
    end
    return nil
  end

  def get_equal(word)
    if has_equal?(word)
      equal_words=[]
      codes = get_ids_by_word(word)
      codes.each{ |code|
        if code[-1]=="#"
          equal_words << get_atom_by_id(code)
        end
      }
      return equal_words
    end
    return nil
  end
  # level 0,1,2,3,4层级 对应同义词词林划分的A，a，01，A，01=五个层次
  def get_similar(word,level=4)
  	codes = get_ids_by_word(word)
    similar=[]
    codes.each{ |code|
      mini_similar=[]
      findstring = gen_findstring(code, level+1)
      similar_codes=@codesTrie.wildcard(findstring)
      similar_codes.each{|item|
        get_atom_by_id(item).each{|term|
          mini_similar << term
        }
      }
      similar << mini_similar
    }
    if similar.size > 0
    	return similar
    else
    	return nil
    end
  end

  def gen_findstring(code,level)
    frame = cut_code(code)
    (level).upto(4){|i|
    	0.upto(frame[i].size-1){ |j|
    		frame[i][j]='.'
    	}
    }
    combine_code(frame)
  end

  def cut_code(code)
    frame=[code[0],code[1],code[2..3],code[4],code[5..7]]
    return frame
  end

  def combine_code(frame)
    m=""
    frame.each{|seg|
      m << seg
    }
    return m
  end

  def compare_code(codeA,codeB) #return the fisrt level that diffrent[0,4] if same return 5
    frameA=cut_code(codeA)
    frameB=cut_code(codeB)
    0.upto(frameA.length-1){ |i|
      if frameA[i].eql?(frameB[i]) == false
        return i
      end
    }
    return 5
  end

  def get_dist_by_id(codeA,codeB)
  	alpha=10.0/5
  	n = compare_code(codeA,codeB)
  	(alpha*(5-n)).round
  end

  def get_sim_by_id(codeA,codeB)
   	n = compare_code(codeA,codeB)
    str = codeA.clone   
    if n==0
      _sim = factor[0]
    elsif n==5
      if codeA[-1] == "="
        _sim = factor[5]
      elsif codeA[-1] == "#"
        _sim = factor[6]
      elsif codeA[-1] == "@"
        _sim = factor[5]
      end
  	elsif n < 5
  	  findstring=gen_findstring(str,n)
      node_num = @codesTrie.wildcard(findstring).size
      k = (@codesIndex[codeA]-@codesIndex[codeB]).abs
      _sim = factor[n]*(Math.cos(node_num*Math::PI/180)).abs*((node_num-k+1)*1.0/node_num)
    end
    return _sim
  end 
  
  def dist(wordA,wordB)
    alpha=10.0/5
    shortest_Pair = Pairs.new(100,"","")
    codeAs = get_ids_by_word(wordA)
    codeBs = get_ids_by_word(wordB)

    codeAs.each{ |codeA|
      codeBs.each{ |codeB|
        n = compare_code(codeA,codeB)
          distance = (alpha*(5-n)).round
        if distance < shortest_Pair.value
          shortest_Pair.value = distance
          shortest_Pair.X_id = codeA
          shortest_Pair.Y_id = codeB
        end
      }
    }
    return shortest_Pair
  end

  def sim(wordA,wordB)
    factor=[0.02,0.65,0.8,0.9,0.96,1,0.5]#0,1,2,3,4,5各层参数
    longest_Pair = Pairs.new(-1,"","")
    codeAs = get_ids_by_word(wordA)
    codeBs = get_ids_by_word(wordB)

    codeAs.each{ |codeA|
      codeBs.each{ |codeB|
        n = compare_code(codeA,codeB)
        str = codeA.clone   
        if n==0
          _sim = factor[0]
        elsif n==5
          if codeA[-1] == "="
            _sim = factor[5]
          elsif codeA[-1] == "#"
            _sim = factor[6]
          elsif codeA[-1] == "@"
            _sim = factor[5]
          end
      	elsif n < 5
      	  findstring=gen_findstring(str,n)
          node_num = @codesTrie.wildcard(findstring).size
          k = (@codesIndex[codeA]-@codesIndex[codeB]).abs
          _sim = factor[n]*(Math.cos(node_num*Math::PI/180)).abs*((node_num-k+1)*1.0/node_num)
        end
        
        if _sim > longest_Pair.value
          longest_Pair.value = _sim
          longest_Pair.X_id = codeA
          longest_Pair.Y_id = codeB
        end
      }
    }
    longest_Pair.value = ("%1.5f" % longest_Pair.value).to_f
    return longest_Pair
  end

end
